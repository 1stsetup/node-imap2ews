...
console.log("--- startTLS ---");
	...
	console.log("Going to unpipe.");	

	this._socket.unpipe(this);
	this.unpipe(this._socket);

	console.log("Before Create secure pair.");	

	var pair = require("tls").createSecurePair(this._sslContext, true, false, false);
	
	console.log("Create secure pair.");	
	pair.encrypted.pipe(this._socket);
	this._socket.pipe(pair.encrypted);

	pair.fd = this._socket.fd;
	var cleartext = pair.cleartext;
	cleartext.socket = this._socket;
...
console.log("_processString: command:", command);
	...
	if (IMAP_COMMANDS[command]["responseFunc"]) {
		IMAP_COMMANDS[command].responseFunc(this, tag);
	}
	else {
